...
import { catchError, debounceTime, distinctUntilChanged, map, merge, of, startWith, Subscription, switchMap } from 'rxjs';
import { RolesResponse, UniversalPaginationParameters } from '../../../core/models';
import { LoadingAnimation, queryParamBuilder } from '../../../core/utils';
import { RolesService } from './roles.service';

@Component({
  selector: 'mpa-hub-roles',
...
export class RolesComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  displayedColumns: string[] = ['no', 'name', 'level', 'userCount'];
  dataSource: RolesResponse[] = [];
  ...
  tableAction(element: RolesResponse, type: string): void {
